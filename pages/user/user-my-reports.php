...
try {
    ...
    $sql = <<<'SQL'
        ...
        WHERE r.is_deleted = FALSE AND r.user_id = ?
    SQL;
    ...
    $stmt->execute();
    $myReports = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

    if ($myReports !== []) {
        $reportIds = array_map(fn($r) => (int) $r['report_id'], $myReports);
    ...
    $loadError = 'Unable to load your reports right now.';
}
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <main style="display: flex; flex-direction: column; gap: var(--space-medium);">
        ...
        <?php if ($loadError !== null): ?>
            <section class="post" style="padding: 16px;">
                <h2 style="margin-bottom: 8px; color: var(--colorRed);">Unable to load your reports</h2>
                <p style="margin-bottom: 0;"><?php echo escape_html($loadError); ?></p>
            </section>
        <?php elseif ($myReports === []): ?>
            <section class="post" style="padding: 32px; text-align: center;">
                ...
                <h2 style="margin-bottom: 8px;">You haven't submitted any reports yet.</h2>
                <p style="margin-bottom: 0; color: var(--color4);">Click "Create Report" on the sidebar to log your first traffic incident.</p>
            </section>
        <?php else: ?>
                    ...
                    <section class="post">
                        ...
                        <div class="post-buttons">
                            <div class="post-buttons-left">
                                ...
                                <!-- EDIT REPORT LINK -->
                                <?php if ($status !== 'Resolved' && $status !== 'Verified'): ?>
                                    <button class="post-edit" onclick="event.preventDefault(); window.location.href='user-edit-report.php?id=<?php echo $reportId; ?>';" style="background-color: var(--color3); color: var(--colorText); cursor: pointer; padding: 4px 12px; font-weight: 600;">
