...
try {
    ...
    if ($currentUserId !== null) {
        $reportData = fetch_reports_and_media_by_author($db, $currentUserId, $currentUserId);
        $reports = $reportData['reports'];
        $mediaByReport = $reportData['mediaByReport'];
    }
} catch (Throwable $error) {
    $errorMessage = 'Unable to load your reports right now.';
}
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
<?php render_activity_navigation($categories, 'my-reports'); ?>
<div class="activity-main"><main>
    <header class="activity-heading"><h1>My Reports</h1><p>Reports you have submitted and their current official status.</p></header>
    <?php if ($errorMessage !== null): ?>
        <section class="activity-panel"><h2>Reports unavailable</h2><p><?= activity_layout_escape($errorMessage) ?></p></section>
    <?php elseif ($reports === []): ?>
        <section class="activity-panel"><h2>No reports yet</h2><p><a href="user-create-report.php">Create a report</a> to start tracking local incidents.</p></section>
    <?php else: ?>
        <?php foreach ($reports as $report): ?>
            <?php $reportId = (int) $report['report_id']; ?>
            <?php render_report_card($report, $mediaByReport[$reportId] ?? [], 'user-report-details.php?id=' . $reportId, 'report-action.php', '../../', 'user-edit-report.php?id=' . $reportId); ?>
        <?php endforeach; ?>
    <?php endif; ?>
</main></div>
